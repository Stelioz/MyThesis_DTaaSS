...
def process_csv(file_path):
    ...
    for i in range(1, 8):  # Loop through Camera1 to Camera6
    ...
    grouped_result = result_data.groupby("Rank").agg(
        {
            ...
            **{f"Camera{i}_Performance": "max" for i in range(1, 8)}  # Keep max performance for each camera
        }
    )
...
file_path = "output/7c_model_morning_flexsim_data.csv"

# Process the CSV file and calculate performances
result_data = process_csv(file_path)

# Save the result to a new CSV file
result_data.to_csv("output/7c_model_morning_camera_performances.csv", index=False)
